...
class Conv2D(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.kernel = self.kernel_initializer(shape=(self.kernel_size[0], self.kernel_size[1], input_shape[3], self.filters), 
        dtype=tf.float32)
        self.bias = self.bias_initializer(shape=(1,), dtype=tf.float32) #checking shape
        return 
    ...
    def call(self, inputs, training=False): 
        ...
        output = tf.nn.convolution(input=inputs, filters=self.kernel, padding=self.padding,strides=self.strides)
...
class BatchNormalization(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.center:
            beta = self.beta_initializer(shape=(input_shape[self.axis],))
        if self.scale:
            gamma = self.gamma_initializer(shape=(input_shape[self.axis],))

        moving_mean = self.moving_mean_initializer(shape=(input_shape[self.axis],))
        moving_variance = self.moving_variance_initializer(shape=(input_shape[self.axis],))
        ...
        self.moving_mean = tf.Variable(initial_value = moving_mean, trainable= False, dtype=tf.float32)
        self.moving_variance = tf.Variable(initial_value = moving_variance, trainable= False, dtype=tf.float32)
        return 

    def call(self, inputs, training=False):
        self.beta.assign(self.beta_constraint(self.beta))
        self.gamma.assign(self.gamma_constraint(self.gamma))
       
        if training:
            bmu, bvar = tf.nn.moments(inputs, range(len(inputs.shape) - 1)) #batch average and variance
            bstd = tf.sqrt(tf.math.add(bvar,self.epsilon))
          
            self.moving_mean.assign(tf.math.add(tf.math.multiply(self.moving_mean,self.momentum), 
            (tf.math.multiply(bmu, (1 - self.momentum)))))
            self.moving_variance.assign(tf.math.add(tf.math.multiply(self.moving_variance,self.momentum), 
            (tf.math.multiply(bvar, (1 - self.momentum)))))
            
            inputs = tf.Variable(inputs, dtype=tf.float32)
            inputs.assign(tf.math.divide(tf.math.subtract(inputs, bmu), bstd))
            inputs.assign(tf.math.multiply(inputs,self.gamma))
            inputs.assign(tf.math.add(inputs,self.beta))
            ...
            inputs = tf.Variable(inputs, dtype=tf.float32)
            inputs.assign(tf.math.divide(tf.math.subtract(inputs,self.moving_mean),
            tf.sqrt(tf.math.add(self.moving_variance, self.epsilon))))
    ...
    def call(self, inputs, training=False):
        if training:
            ...
            if self.noise_shape:
                ...
                shape = inputs.shape
            dropout_layer = tf.random.uniform(shape, minval=0.0, maxval=1.0, dtype=tf.float32, seed = self.seed)
            dropout_layer = tf.keras.backend.cast(dropout_layer > self.rate, tf.keras.backend.floatx())
            inputs = tf.math.multiply(inputs, dropout_layer)
            scale = tf.math.scalar_mul(1- self.rate, tf.keras.initializers.Ones()(shape=inputs.shape, dtype=tf.float32))
            inputs = tf.math.divide(inputs, scale)
